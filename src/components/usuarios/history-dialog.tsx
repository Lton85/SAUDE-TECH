...
export function HistoryUsuarioDialog({ isOpen, onOpenChange, usuario }: HistoryDialogProps) {
  if (!usuario || !usuario.historico) return null;
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
           ...
           <DialogDescription>
            Acompanhe o registro de criação e a última alteração do cadastro de <span className="font-semibold text-primary">{usuario.nome}</span>.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
            <InfoCard 
                ...
                user={usuario.historico.criadoPor}
                date={usuario.historico.criadoEm}
            />
            <InfoCard 
                icon={Pencil}
                title="Última Alteração"
                user={usuario.historico.alteradoPor}
                date={usuario.historico.alteradoEm}
            />
        </div>
