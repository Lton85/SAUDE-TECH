...
export function HistoryDepartamentoDialog({ isOpen, onOpenChange, departamento }: HistoryDialogProps) {
  if (!departamento || !departamento.historico) return null;
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
           ...
           <DialogDescription>
            Acompanhe o registro de criação e a última alteração do cadastro de <span className="font-semibold text-primary">{departamento.nome}</span>.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
            <InfoCard 
                ...
                user={departamento.historico.criadoPor}
                date={departamento.historico.criadoEm}
            />
            <InfoCard 
                icon={Pencil}
                title="Última Alteração"
                user={departamento.historico.alteradoPor}
                date={departamento.historico.alteradoEm}
            />
        </div>
