...
export function HistoryEnfermeiroDialog({ isOpen, onOpenChange, enfermeiro }: HistoryDialogProps) {
  if (!enfermeiro || !enfermeiro.historico) return null;
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Histórico do(a) Enfermeiro(a)
          </DialogTitle>
           <DialogDescription>
            Acompanhe o registro de criação e a última alteração do cadastro de <span className="font-semibold text-primary">{enfermeiro.nome}</span>.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
            <InfoCard 
                ...
                user={enfermeiro.historico.criadoPor}
                date={enfermeiro.historico.criadoEm}
            />
            <InfoCard 
                icon={Pencil}
                title="Última Alteração"
                user={enfermeiro.historico.alteradoPor}
                date={enfermeiro.historico.alteradoEm}
            />
        </div>
